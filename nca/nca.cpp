...
std::vector<uint8_t> nca::compress(const std::vector<uint8_t>& data)
{
	...
	std::map<size_t, std::span<uint8_t>> windows;
	std::map<size_t, const std::span<uint8_t>&> duplicate_windows;
	...
	std::map<uint64_t, std::pair<const std::span<uint8_t>&, std::set<size_t>>> compression_map;
	...
	const auto next_symbol = [&last_symbol, &windows]() -> uint64_t
	{
		while (last_symbol < std::numeric_limits<uint64_t>::max())
		{
			...
			{
				for (size_t i = 0; i < window_size - sizeof(last_symbol) + 1; i++)
				{
